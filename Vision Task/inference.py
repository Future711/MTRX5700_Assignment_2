...
if __name__ == "__main__":
    ...
    image_path = 'Sign Images'

    for img in os.listdir(image_path):
        img_path = os.path.join(image_path, img)
        predicted_idx = inference(model_path, img_path)
        print(f"Image: {img} - Predicted Class: {CLASS_NAMES[predicted_idx]}")

        img = cv2.imread(img_path)
        img = cv2.resize(img, (400, 400))
        cv2.imshow(f"Prediction: {CLASS_NAMES[predicted_idx]}", img)
        cv2.waitKey(0)
