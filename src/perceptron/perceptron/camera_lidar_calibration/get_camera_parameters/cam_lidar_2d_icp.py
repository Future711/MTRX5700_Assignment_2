...
def main():
  ...
  parser.add_argument("--calibration", default="calibration_example.json", help="Calibration JSON file to read intrinsics from and write extrinsics to (default: calibration_example.json).")
  ...
  print(f"K:\n{camera_k}")
  print(f"distortion: {camera_dist}")
  ...
  # Extract x, y, yaw from the Open3D ICP result and write extrinsics to calibration JSON
  # icp_result is a 2D rigid transform: [[cos, -sin, tx], [sin, cos, ty], [0, 0, 1]]
  yaw = float(np.arctan2(icp_result[1, 0], icp_result[0, 0]))
  x_offset = float(icp_result[2, 0]) if icp_result.shape[0] > 2 else float(icp_result[0, 2])
  y_offset = float(icp_result[1, 2])
  z_offset = 0.0

  # Construct the 4x4 T_cam_lidar from calibration parameters
